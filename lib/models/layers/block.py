...
class CrossBlock(nn.Module):
    ...
    def forward(self, x_q: Tensor, x_kv: Tensor, pos=None) -> Tensor:
        def attn_residual_func(x_q: Tensor,x_kv: Tensor, pos=None) -> Tensor:
            return self.ls1(self.attn(self.norm1_q(x_q),self.norm1_kv(x_kv), pos=pos))
        ...
        if self.training and self.sample_drop_ratio > 0.0:
            x = x + self.drop_path1(attn_residual_func(x_q, x_kv, pos=pos))
            x = x + self.drop_path1(ffn_residual_func(x_q, x_kv))  # FIXME: drop_path2
        else:
            x = x_q + attn_residual_func(x_q, x_kv, pos=pos)
            x = x + ffn_residual_func(x)
        return x
